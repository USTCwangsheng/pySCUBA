...
//	static std::string attrib_name;
	...
	static BackBoneSite read(IT &lineit) {
		...
		std::vector<std::string> line1 = NSPdataio::parseline(*lineit++,
				std::vector<int>());
		bs.pdbid = NSPutils::trim_copy(line1[0]);
		bs.chainid = line1[1][0];
		bs.resid = stoi(line1[2]);
		bs.resname = NSPutils::trim_copy(line1[3]);
		bs.resseq = stoi(line1[4]);
		bs.sscode = line1[5][0];
		for (int i = 0; i < NCRD; ++i) {
			bs.data_[i] = stod(line1[6 + i]);
		}
		for (int i = NCRD; i < NCRD + 12; i += 3) {
			line1 = NSPdataio::parseline(*lineit++, std::vector<int>());
			;
			for (int j = 0; j < 3; j++)
				bs.data_[i + j] = stod(line1[j]);
		}
		return bs;
	}
	...
	void changecrd(const std::vector<double> &crd) {
		assert(crd.size() >= 12);
		for (int i = 0; i < 12; ++i)
			data_[NCRD + i] = crd[i];
	}
	...
	void changecrd(const std::vector<NSPgeometry::XYZ> &crd) {
		assert(crd.size() >= 4);
		int indx = NCRD;
		for (int i = 0; i < 4; i++) {
			data_[indx++] = crd[i].x_;
    ...
    inline bool operator ==(const BackBoneSite &bs){ return false;}
	std::string pdbid{"XXXX"};
