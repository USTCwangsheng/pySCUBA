...
public:
	...
	void parsetxt(const std::string & line) {
		...
		if (key_len == std::string::npos)
			return;
		//       key_len +=1;
		std::string key = NSPutils::trim_copy(line.substr(0, key_len));
		PARA val;
...
#endif
	/*!transform a string into a value of type T
	 *
	 */
	template<typename T>
	void getparametervalue(const std::string & str, T * val) {
		//      std::cout <<"val: " <<str <<std::endl;
		(*val) = NSPutils::toValue<T>(NSPutils::trim_copy(str));
	}
...
struct ParameterSet {
	...
	void initdefaultkeys(const std::map<std::string,double> &doublepars,
		...
		InitDoubleKeys(NSPdstl::getkeyvec(doublepars));
		InitIntKeys(NSPdstl::getkeyvec(intpars));
		InitStringKeys(NSPdstl::getkeyvec(stringpars));
		InitDoubleVecKeys(NSPdstl::getkeyvec(doublevecpars));
		InitStringVecKeys(NSPdstl::getkeyvec(stringvecpars));
		InitIntVecKeys(NSPdstl::getkeyvec(intvecpars));
		initialized=true;
		for (auto &kv : doublepars) {
				DoublePar.insert(kv.first, kv.second);
		}
		for (auto &kv : intpars) {
				IntPar.insert(kv.first, kv.second);
		}
		for (auto &kv : stringpars) {
				StringPar.insert(kv.first, kv.second);
		}
		for (auto &kv : doublevecpars) {
				DoubleVecPar.insert(kv.first, kv.second);
		}
		for (auto &kv : intvecpars) {
				IntVecPar.insert(kv.first, kv.second);
		}
		for (auto &kv : stringvecpars) {
				StringVecPar.insert(kv.first, kv.second);
		}
	}
	int adjustvalues(const std::vector<std::string> & controllines){
		assert(initialized);
		int nsuccess = 0;
...
public:
	...
	static void initdefaultkeyvals(std::string controlname,const std::map<std::string,double> &doublepars,
		...
		pset.initdefaultkeys(doublepars,
				stringpars,
				intpars,
				doublevecpars,
				stringvecpars,
	...
	static int adjustvalues(std::string controlname,const std::vector<std::string> &controllines) {
		...
		return pset.adjustvalues(controllines);
	}
