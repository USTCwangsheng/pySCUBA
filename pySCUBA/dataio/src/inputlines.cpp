...
std::pair<std::string,std::string> NSPdataio::keyvalstrings(const std::string &line){
  auto res=std::make_pair<std::string,std::string>("","");
    ...
    res.first=NSPutils::trim_copy(line.substr(0,key_len));
    res.second=NSPutils::trim_copy(line.substr(key_len+1));
    return res;
}
...
std::vector<std::string> NSPdataio::parseline(
    ...
    if (cols.empty()) {
        NSPutils::split(inputline, parsedline, " ,\t");
        }   
    else {
        NSPutils::split(inputline, parsedline, cols);
    }
    return parsedline;
}
