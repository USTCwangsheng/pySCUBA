...
struct XYZ {
	...
	bool operator == (const XYZ &p) const {
		if(x_==p.x_&& y_==p.y_ && z_==p.z_) return true;
		return false;
	}
...
inline std::vector<double> XYZvtodoublev(const std::vector<XYZ> &crds){
	std::vector<double> res;
	for(auto &r:crds) {
		res.push_back(r.x_);res.push_back(r.y_);res.push_back(r.z_);
	}
	return res;
}
inline std::vector<XYZ> doublevtoXYZv(const std::vector<double> &crds){
	std::vector<NSPgeometry::XYZ> res;
	for(int i=0;i<crds.size()/3;++i){
		res.push_back(XYZ{crds[3*i],crds[3*i+1],crds[3*i+2]});
	}
	return res;
}
